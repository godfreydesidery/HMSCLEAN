...
| Consultation transfer between clinics | ✅ | Gap-audit OPC-1 (V74) reworked this to the faithful **two-phase pending hand-off**. The treating doctor raises a PENDING `ConsultationTransfer` against a target **clinic only** (`POST /encounters/consultations/uid/{uid}/transfer` with `targetClinicUid` + `reason`); gates: source IN_PROGRESS, no other pending transfer for the patient, target clinic active & ≠ source, and **no un-acted (REQUESTED order / PENDING prescription) on the source**. The source flips to TRANSFERRED (no receiver yet). The pending queue (`GET /encounters/consultations/transfers?status=PENDING`) is worked by reception, which **accepts** (`POST .../transfers/uid/{uid}/accept` choosing the receiving clinician, affiliation-gated) — booking a fresh consultation at the target (inherits feeSettled, no re-charge), COMPLETEs the transfer, links the source — or the initiating doctor **cancels** (`POST .../transfers/uid/{uid}/cancel`), reverting the source to IN_PROGRESS. Optimistic-locked so accept/cancel races resolve first-writer-wins. (Superseded the old Phase 44 immediate-booking design.) |
