...
| Patient consumable chart | ✅ | Phase 41 — `ConsumableIssue` aggregate per admission with snapshot unit cost. `POST /encounters/admissions/uid/{uid}/consumables` + GET for the chart. Admission invoice generator picks rows up as `CONSUMABLE`-kind lines. Stock decrement at the source store / pharmacy is still a separate (manual) step — the audit + billing layer is done; the inventory integration can ride on a later phase. |
