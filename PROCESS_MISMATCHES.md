...
| **M23** | Ward-day charge accrues daily (interim billing point mid-stay) | Computed on-demand at invoice generation. | Low | 📝 acceptable — `generateForAdmission` computes ward-days to *now*, so a cashier pulling the admission invoice mid-stay gets the accrued charge: on-demand generation **is** the interim billing point. A scheduled daily-accrual job is an operational nicety, not a flow gate. |
