...
| **M24** | Payroll `PayrollDetail` itemises tax/insurance/loan deductions | Snapshot gross/deductions/net only; `PayrollComponent`/`Band` tables exist but aren't wired into the period flow. | Medium | ⬜ remaining — larger feature on the (deliberately) skeleton payroll module; the `PayrollComponent` compute + "auto-prefill" UI already exist, so the gap is persisting the per-component breakdown onto the item. Recommended as a focused follow-up. |
