...
import { INVOICE_STATUSES, InvoiceStatus, InvoiceSummary } from './invoice.types';
...
export class InvoiceListComponent {
  ...
  private readonly result = toSignal(
    this.refresh$.pipe(
      ...
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load invoices.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );
  ...
  setStatusFilter(v: InvoiceStatus | 'ALL'): void { this.statusFilter.set(v); this.page.set(0); this.refresh$.next(); }
  ...
  view(s: InvoiceSummary): void { void this.router.navigate(['/billing', s.uid]); }
