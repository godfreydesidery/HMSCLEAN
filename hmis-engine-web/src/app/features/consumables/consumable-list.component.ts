...
export class ConsumableListComponent {
  ...
  private readonly result = toSignal(
    this.refresh$.pipe(
      ...
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load consumables.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );
  ...
  openCreate(): void { const r = this.modal.open(ConsumableFormComponent, { size: 'lg', backdrop: 'static' }); r.closed.subscribe(() => this.refresh$.next()); }
  openEdit(c: Consumable): void { const r = this.modal.open(ConsumableFormComponent, { size: 'lg', backdrop: 'static' }); (r.componentInstance as ConsumableFormComponent).existing = c; r.closed.subscribe(() => this.refresh$.next()); }

  toggleActive(c: Consumable): void {
    this.service.setActive(c.uid, !c.active).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not update consumable.')
    });
  }
  delete(c: Consumable): void {
    if (!globalThis.confirm(`Delete consumable "${c.name}"? This cannot be undone.`)) return;
    this.service.delete(c.uid).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not delete consumable.')
    });
  }
}
