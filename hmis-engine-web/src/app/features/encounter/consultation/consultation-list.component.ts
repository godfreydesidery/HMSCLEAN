...
export class ConsultationListComponent {
  ...
  readonly statusFilter = signal<ConsultationStatus | 'ALL'>('ALL');
  ...
  private readonly result = toSignal(
    this.refresh$.pipe(
      ...
      switchMap(() => {
        ...
        return this.consultationService.search({
          query: this.searchQuery() || undefined,
          status: this.statusFilter() === 'ALL' ? undefined : (this.statusFilter() as ConsultationStatus),
          page: this.page(), size: this.pageSize(), sort: 'bookedAt,desc'
        }).pipe(finalize(() => this.loading.set(false)));
      }),
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load consultations.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );
  ...
  setStatusFilter(v: ConsultationStatus | 'ALL'): void { this.statusFilter.set(v); this.page.set(0); this.refresh$.next(); }
  ...
  start(): void { void this.router.navigate(['/encounters', 'consultations', 'new']); }
  ...
  patientInitials(c: ConsultationSummary): string {
    const parts = (c.patientName ?? '').split(' ').filter((p) => p.length > 0);
