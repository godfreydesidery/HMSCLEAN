...
export class EmployeeListComponent {
  ...
  readonly statusFilter = signal<EmploymentStatus | 'ALL'>('ALL');
    ...
    this.refresh$.pipe(
      ...
      switchMap(() => {
        ...
        return this.employeeService.search({
          ...
          status: this.statusFilter() === 'ALL' ? undefined : (this.statusFilter() as EmploymentStatus),
          page: this.page(), size: this.pageSize(), sort: 'lastName,asc'
        }).pipe(finalize(() => this.loading.set(false)));
      }),
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load employees.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );
  ...
  setStatusFilter(v: EmploymentStatus | 'ALL'): void { this.statusFilter.set(v); this.page.set(0); this.refresh$.next(); }
  ...
  view(e: Employee): void { void this.router.navigate(['/hr/employees', e.uid]); }
