...
@Component({
  selector: 'app-administration-route-form',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './administration-route-form.component.html'
})
export class AdministrationRouteFormComponent implements OnInit {
  @Input() existing: AdministrationRoute | null = null;
  ...
  get title(): string { return this.isEdit ? 'Edit administration route' : 'New administration route'; }
  ...
  submit(): void {
    ...
    req$.pipe(finalize(() => this.submitting.set(false))).subscribe({
      ...
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not save administration route.')
    });
  }
}
