...
export class DiagnosisListComponent {
  ...
  private readonly result = toSignal(
    this.refresh$.pipe(
      ...
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load diagnoses.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );
  ...
  openCreate(): void { const r = this.modal.open(DiagnosisFormComponent, { size: 'lg', backdrop: 'static' }); r.closed.subscribe(() => this.refresh$.next()); }
  openEdit(d: DiagnosisType): void { const r = this.modal.open(DiagnosisFormComponent, { size: 'lg', backdrop: 'static' }); (r.componentInstance as DiagnosisFormComponent).existing = d; r.closed.subscribe(() => this.refresh$.next()); }

  toggleActive(d: DiagnosisType): void {
    this.service.setActive(d.uid, !d.active).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not update diagnosis.')
    });
  }
  delete(d: DiagnosisType): void {
    if (!globalThis.confirm(`Delete diagnosis "${d.name}"? This cannot be undone.`)) return;
    this.service.delete(d.uid).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not delete diagnosis.')
    });
  }
}
