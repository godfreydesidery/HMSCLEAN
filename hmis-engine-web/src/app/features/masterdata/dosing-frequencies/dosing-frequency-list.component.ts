...
import { DosingFrequencyFormComponent } from './dosing-frequency-form.component';
import { DosingFrequencyService } from './dosing-frequency.service';
import { DosingFrequency } from './dosing-frequency.types';

@Component({
  selector: 'app-dosing-frequency-list',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, NgbDropdownModule],
  templateUrl: './dosing-frequency-list.component.html'
})
export class DosingFrequencyListComponent {
  ...
  private readonly result = toSignal(
    this.refresh$.pipe(
      ...
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load dosing frequencies.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );
  ...
  openCreate(): void { const r = this.modal.open(DosingFrequencyFormComponent, { size: 'lg', backdrop: 'static' }); r.closed.subscribe(() => this.refresh$.next()); }
  openEdit(d: DosingFrequency): void { const r = this.modal.open(DosingFrequencyFormComponent, { size: 'lg', backdrop: 'static' }); (r.componentInstance as DosingFrequencyFormComponent).existing = d; r.closed.subscribe(() => this.refresh$.next()); }

  toggleActive(d: DosingFrequency): void {
    this.service.setActive(d.uid, !d.active).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not update dosing frequency.')
    });
  }
  delete(d: DosingFrequency): void {
    if (!globalThis.confirm(`Delete dosing frequency "${d.name}"? This cannot be undone.`)) return;
    this.service.delete(d.uid).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not delete dosing frequency.')
    });
  }
}
