...
export class InsuranceListComponent {
  ...
  private readonly result = toSignal(
    this.refresh$.pipe(
      ...
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load insurance providers.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );
  ...
  openCreate(): void { const r = this.modal.open(InsuranceFormComponent, { size: 'lg', backdrop: 'static' }); r.closed.subscribe(() => this.refresh$.next()); }
  openEdit(i: InsuranceProvider): void { const r = this.modal.open(InsuranceFormComponent, { size: 'lg', backdrop: 'static' }); (r.componentInstance as InsuranceFormComponent).existing = i; r.closed.subscribe(() => this.refresh$.next()); }

  toggleActive(i: InsuranceProvider): void {
    this.service.setActive(i.uid, !i.active).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not update insurance.')
    });
  }
  delete(i: InsuranceProvider): void {
    if (!globalThis.confirm(`Delete "${i.name}"? This cannot be undone.`)) return;
    this.service.delete(i.uid).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not delete insurance.')
    });
  }
}
