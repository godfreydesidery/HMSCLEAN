...
export class PharmacyListComponent {
  ...
  private readonly result = toSignal(
    this.refresh$.pipe(
      ...
      switchMap(() => {
        ...
        return this.pharmacyService.search({
      ...
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load pharmacies.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );

  readonly pharmacies = computed(() => this.result()?.content ?? []);
  ...
  openCreate(): void {
    const r = this.modal.open(PharmacyFormComponent, { size: 'lg', backdrop: 'static' });
    r.closed.subscribe(() => this.refresh$.next());
  }

  openEdit(p: Pharmacy): void {
    const r = this.modal.open(PharmacyFormComponent, { size: 'lg', backdrop: 'static' });
    (r.componentInstance as PharmacyFormComponent).existing = p;
    r.closed.subscribe(() => this.refresh$.next());
  }

  toggleActive(p: Pharmacy): void {
    this.pharmacyService.setActive(p.uid, !p.active).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not update pharmacy.')
    });
  }

  delete(p: Pharmacy): void {
    if (!globalThis.confirm(`Delete pharmacy "${p.name}"? This cannot be undone.`)) return;
    this.pharmacyService.delete(p.uid).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not delete pharmacy.')
    });
  }
}
