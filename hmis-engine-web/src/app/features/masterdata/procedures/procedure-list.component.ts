...
export class ProcedureListComponent {
  ...
  private readonly result = toSignal(
    this.refresh$.pipe(
      ...
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load procedures.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );
  ...
  toggleActive(p: ProcedureType): void {
    this.service.setActive(p.uid, !p.active).subscribe({
      ...
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not update procedure.')
    });
  }
  delete(p: ProcedureType): void {
    if (!globalThis.confirm(`Delete procedure "${p.name}"? This cannot be undone.`)) return;
    this.service.delete(p.uid).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not delete procedure.')
    });
  }
}
