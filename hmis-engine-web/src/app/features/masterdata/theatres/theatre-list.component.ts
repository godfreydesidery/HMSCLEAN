...
export class TheatreListComponent {
  ...
  private readonly result = toSignal(
    this.refresh$.pipe(
      ...
      switchMap(() => {
        ...
        return this.theatreService.search({
      ...
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load theatres.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );

  readonly theatres = computed(() => this.result()?.content ?? []);
  ...
  openCreate(): void {
    const r = this.modal.open(TheatreFormComponent, { size: 'lg', backdrop: 'static' });
    r.closed.subscribe(() => this.refresh$.next());
  }

  openEdit(t: Theatre): void {
    const r = this.modal.open(TheatreFormComponent, { size: 'lg', backdrop: 'static' });
    (r.componentInstance as TheatreFormComponent).existing = t;
    r.closed.subscribe(() => this.refresh$.next());
  }

  toggleActive(t: Theatre): void {
    this.theatreService.setActive(t.uid, !t.active).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not update theatre.')
    });
  }

  delete(t: Theatre): void {
    if (!globalThis.confirm(`Delete theatre "${t.name}"? This cannot be undone.`)) return;
    this.theatreService.delete(t.uid).subscribe({
      next: () => this.refresh$.next(),
      error: (err) => this.errorMessage.set(err?.error?.message ?? 'Could not delete theatre.')
    });
  }
}
