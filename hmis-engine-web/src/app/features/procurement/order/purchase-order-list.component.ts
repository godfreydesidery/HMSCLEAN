...
import {
  PURCHASE_ORDER_STATUSES, PurchaseOrder, PurchaseOrderStatus, PurchaseOrderSummary
...
export class PurchaseOrderListComponent {
    ...
    this.refresh$.pipe(
      ...
      switchMap(() => {
        ...
        return this.purchaseOrderService.search({
          ...
          status: this.statusFilter() === 'ALL' ? undefined : (this.statusFilter() as PurchaseOrderStatus),
          page: this.page(), size: this.pageSize(), sort: 'createdAt,desc'
        }).pipe(finalize(() => this.loading.set(false)));
      }),
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load purchase orders.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );
  ...
  setStatusFilter(v: PurchaseOrderStatus | 'ALL'): void { this.statusFilter.set(v); this.page.set(0); this.refresh$.next(); }
  ...
  openCreate(): void {
    const r = this.modal.open(PurchaseOrderCreateComponent, { size: 'lg', backdrop: 'static' });
    r.closed.subscribe((created: PurchaseOrder | undefined) => {
      if (created) void this.router.navigate(['/procurement/orders', created.uid]);
    });
  }

  view(o: PurchaseOrderSummary): void { void this.router.navigate(['/procurement/orders', o.uid]); }
