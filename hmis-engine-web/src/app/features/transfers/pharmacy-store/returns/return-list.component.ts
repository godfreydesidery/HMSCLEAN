...
import { RETURN_STATUSES, ReturnStatus, ReturnSummary, returnBadgeClass, returnLabel } from './return.types';
...
export class ReturnListComponent {
  ...
  readonly statusFilter = signal<ReturnStatus | 'ALL'>('ALL');
  ...
  private readonly result = toSignal(
    this.refresh$.pipe(
      ...
      switchMap(() => {
        ...
        return this.returnService.search({
          query: this.searchQuery() || undefined,
          status: this.statusFilter() === 'ALL' ? undefined : (this.statusFilter() as ReturnStatus),
          page: this.page(), size: this.pageSize(), sort: 'createdAt,desc'
        }).pipe(finalize(() => this.loading.set(false)));
      }),
      tap({ error: (err) => { this.errorMessage.set(err?.error?.message ?? 'Could not load returns.'); this.loading.set(false); } }),
      takeUntilDestroyed()
    ),
    { initialValue: null }
  );

  readonly returns = computed(() => this.result()?.content ?? []);
  ...
  setStatusFilter(v: ReturnStatus | 'ALL'): void { this.statusFilter.set(v); this.page.set(0); this.refresh$.next(); }
  ...
  view(o: ReturnSummary): void { void this.router.navigate([o.uid], { relativeTo: this.route }); }
